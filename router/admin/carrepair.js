...
carRepairAdminRoute.get("/", async (req, res) => {
  try {
    ...
    const listCarRepair = await CarRepair.find({ $and: [query, listKeyword] })
      .limit(pagesize)
      .skip(pagesize * page)
      .sort(`${sortBy}`).populate({ path: "car", select: "plate" })
    if (listCarRepair)
...
carRepairAdminRoute.get("/:id", async (req, res) => {
  try {
    const { id } = req.params
    const carrepairs = await CarRepair.findOne({_id: id});
    if (carrepairs)
...
/**
 * @route get /api/admin/carrepair/carInfo/getInfoCar
 * @description get info car to repair
 * @access private
 */
carRepairAdminRoute.get("/carInfo/getInfoCar", async (req, res) => {
  try {
    ...
    return sendSuccess(res, "Get info car to repair successful", arrayCar)
  } catch (error) {
    console.log(error)
    return sendServerError(res)
  }
