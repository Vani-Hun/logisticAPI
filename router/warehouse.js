...
import { verifyToken, verifyStorekeeper } from "../middleware/verify.js"
...
warehouseRoute.get('/',
    async (req, res) => {
        try {
            ...
            const warehouses = await Warehouse.find({ $and: [query, keywordCondition] }).limit(pageSize).skip(pageSize * page).sort(`${sortBy}`).populate({
                path: "storekeeper",
                select: ["code", "name"],
            })
...
warehouseRoute.get('/:id',
    async (req, res) => {
        try {
            const { id } = req.params
            let warehouse = await Warehouse.findById(id).populate({
                path: "storekeeper",
                select: "code",
            })
            if (!warehouse) { return sendError(res, "No information found.") }
            return await sendSuccess(res, "get warehouse successful.", warehouse)
        } catch (error) {
            return sendServerError(res)
        }
    }
...
* @access private
*/
warehouseRoute.put('/add-inventory/:warehouseId/', verifyToken,
    async (req, res) => {
        try {
            ...
            if (warehouse.storekeeper == user.role) {
...
* @access private
*/
warehouseRoute.put('/update-inventory/:warehouseId', verifyToken,
    async (req, res) => {
        const user = await User.findById(req.decoded.userId);
        if (!user) {
            return sendError(res, "User not found", 404);
